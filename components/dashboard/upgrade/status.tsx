      ...
      case "approved":
        return (
          <Badge className="gap-1 bg-blue-500">
            <CheckCircle className="h-4 w-4" /> {t("statusApproved")}
          </Badge>
        )
      case "rejected":
        return (
          <Badge variant="destructive" className="gap-1">
            <XCircle className="h-4 w-4" /> {t("statusRejected")}
          </Badge>
        )
      case "completed":
        return (
          <Badge className="gap-1 bg-green-500">
            <DollarSign className="h-4 w-4" /> {t("statusCompleted")}
          </Badge>
        )
      default:
            ...
            <Card key={request.request_id}>
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div>
                    <CardTitle>
                      {t("requestNumber", { number: request.request_id.slice(0, 8) })}
                    </CardTitle>
                    <CardDescription>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-green-600">${request.target_plan_price}</span>
                </div>
                {request.admin_notes && (
